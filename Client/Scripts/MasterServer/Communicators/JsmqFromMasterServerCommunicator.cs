...
) : Object, ICommunicator
{
    public void Poll()
    {
        while (true)
        {
            string? base64 = (string?)JavaScript.Eval($"""jsmq.receive("{MasterServerId}")""");
            ...
            if (packet is MasterServerHandshakePacket handshake)
            {
    ...
    {
        byte[] bytes = [..MasterServerId.ToByteArray(), ..packetSerializer.Serialize(packet)];
